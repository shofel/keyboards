...
void leader_end_user(void) {
   ...
   * one key). */
  if (leader_sequence_two_keys(KC_M, KC_L) || leader_sequence_two_keys(KC_DOT, KC_L)) { ru_emit_glyph("$l", 0x20BA); } // ₺ lira  (TRY)
  if (leader_sequence_two_keys(KC_M, KC_R) || leader_sequence_two_keys(KC_DOT, KC_R)) { ru_emit_glyph("$r", 0x20BD); } // ₽ ruble (RUB)
  if (leader_sequence_two_keys(KC_M, KC_E) || leader_sequence_two_keys(KC_DOT, KC_E)) { ru_emit_glyph("$e", 0x20AC); } // € euro  (EUR)
